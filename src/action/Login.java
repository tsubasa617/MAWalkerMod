...
public class Login {
	public static final Action Name = Action.LOGIN;
	...
	public static boolean run(boolean jump) throws Exception {
		...
		Document doc;
		try {
			doc = Process.ParseXMLBytes(result, Name.name());
		} catch (Exception ex) {
			ErrorData.currentDataType = ErrorData.DataType.text;
	...
	private static boolean parse(Document doc) throws Exception {
		try {
			...
			Process.info.gachaTicket = Integer.parseInt(xpath.evaluate(
					"//your_data/gacha_ticket", doc));
			Process.info.gachaPoint = Integer.parseInt(xpath.evaluate(
					"//your_data/gacha_point", doc));
		} catch (Exception ex) {
