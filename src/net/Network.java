...
public class Network {
	...
	public Network() {
		UserAgent = "\tMillion/"
				+ GAME_VERSION.replaceAll("\\.", "")
				+ " (t03gchn; t03gzc; 4.1.2) samsung/t03gzc/t03gchn:4.1.2/JZ054K/N7100ZCDMD3:user/release-keys GooglePlay";

		this.client = new DefaultHttpClient();

		HttpParams hp = this.client.getParams();
		hp.setParameter("http.socket.timeout", Integer.valueOf(0x7530));
		hp.setParameter("http.connection.timeout", Integer.valueOf(0x7530));
	}
	...
	public byte[] ConnectToServer(String url, List<NameValuePair> content,
		...
		AuthScope as = new AuthScope(hp.getURI().getHost(), hp.getURI()
				.getPort());
		CredentialsProvider cp = this.client.getCredentialsProvider();
		UsernamePasswordCredentials upc = new UsernamePasswordCredentials(Auth,
				Key);
		cp.setCredentials(as, upc);
		byte[] b = (byte[]) this.client.execute(hp, new HttpResponseHandler());
